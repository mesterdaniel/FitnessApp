...
export function AdminUsersView({
  ...
  return (
    <div className="space-y-6 relative animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <form className="grid gap-3 rounded-[2rem] bg-background/50 backdrop-blur-xl border border-white/5 p-4 shadow-2xl md:grid-cols-[1fr_180px_auto]">
        <div className="relative">
          ...
          <Input
            ...
            className="h-11 rounded-full border-none bg-black/40 pl-11 text-foreground placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-primary/50"
          />
        </div>
        <Select name="role" defaultValue={role || 'all'}>
          <SelectTrigger className="h-11 rounded-full border-none bg-black/40 px-4 text-foreground focus:ring-1 focus:ring-primary/50">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="rounded-lg border border-white/5 bg-background shadow-2xl text-foreground">
      ...
      {message && (
        <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 text-sm text-emerald-400 font-medium flex items-center animate-in fade-in slide-in-from-top-2">
      ...
      <div className="space-y-3">
        {users.map((u, i) => {
          ...
          return (
            <Card
              key={u.id}
              className={`overflow-hidden rounded-lg border border-white/5 bg-background/50 backdrop-blur-md shadow-xl hover:bg-card/50 transition-colors ${isPending ? 'opacity-70 pointer-events-none' : ''}`}
            ...
            >
              <CardContent className="grid gap-4 p-5 md:grid-cols-[1fr_170px_190px_auto] md:items-center">
                ...
                <Select
                  ...
                  disabled={isPending}
                >
                  <SelectTrigger className={`h-10 rounded-md border-none px-4 text-xs font-bold uppercase tracking-wider ${
                  ...
                  <SelectContent className="rounded-lg border border-white/5 bg-background shadow-2xl">
                ...
                <Dialog>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-lg border border-white/5 bg-background p-2 shadow-2xl text-foreground">
                  ...
                  {selectedUser?.id === u.id && (
                    <DialogContent className="rounded-[2rem] border border-white/10 bg-background/90 backdrop-blur-2xl sm:max-w-[450px] shadow-2xl shadow-black">
                      ...
                      <form action={async (formData) => {
                        ...
                        <div className="grid gap-6 py-6">
                          <div className="space-y-3">
                            <label htmlFor="name" className="text-sm font-semibold text-muted-foreground ml-2">Teljes Név</label>
                            <Input
                              id="name"
                              name="full_name"
                              defaultValue={u.full_name || ''}
                              className="h-12 rounded-md border-none bg-black/40 px-5 text-foreground focus-visible:ring-1 focus-visible:ring-primary/50"
                              placeholder="Kovács János"
                            />
                          </div>
                          <div className="rounded-lg bg-white/5 p-4 space-y-2">
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Rendszer Információk</p>
                            <div className="grid grid-cols-2 gap-2 text-sm">
                              <span className="text-muted-foreground">ID:</span>
                              <span className="text-foreground font-mono text-xs">{u.id}</span>
                              <span className="text-muted-foreground">Regisztráció:</span>
                              <span className="text-foreground">{new Date(u.created_at).toLocaleString('hu-HU')}</span>
                            </div>
                          </div>
                        </div>
                        <DialogFooter className="gap-2 sm:gap-0 pt-4 border-t border-white/10">
                          <DialogClose asChild>
                            <Button type="button" variant="ghost" className="rounded-full hover:bg-white/10 text-muted-foreground">Mégse</Button>
                          </DialogClose>
                          <DialogClose asChild>
      ...
      {users.length === 0 && (
        <div className="rounded-[2rem] bg-background/50 backdrop-blur-xl border border-white/5 p-12 text-center text-muted-foreground flex flex-col items-center gap-4 shadow-xl">
