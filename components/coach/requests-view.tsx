...
export function CoachRequestsView({ initialRequests }: { initialRequests: any[] }) {
  ...
  const pendingRequests = initialRequests.filter(req => req.status === 'pending')
  const historyRequests = initialRequests.filter(req => req.status !== 'pending')
  ...
  function RequestCard({ req, isPendingView }: { req: any, isPendingView: boolean }) {
    ...
    <Tabs defaultValue="pending" className="w-full">
      ...
      <TabsContent value="pending" className="mt-6">
        {pendingRequests.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {pendingRequests.map(req => <RequestCard key={req.id} req={req} isPendingView={true} />)}
          </div>
        )}
      </TabsContent>
      ...
      <TabsContent value="history" className="mt-6">
        {historyRequests.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {historyRequests.map(req => <RequestCard key={req.id} req={req} isPendingView={false} />)}
          </div>
        )}
      </TabsContent>
    </Tabs>
